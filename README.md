...
当前仓库采用 TypeScript monorepo，运行时以 Next.js 为主应用：

- `apps/web`：Next.js Web 应用，同时承接页面和 `/api/*` Route Handler。
- `apps/api`：后端业务模块，由 `apps/web` 的 Next Route Handler 以内嵌 application context 方式调用，不再作为独立 HTTP 服务启动。
