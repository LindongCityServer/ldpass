...
| 登录设备管理    | 已提供设备列表、手动下线设备、同系统最多保留 2 台活动登录设备且超出时自动下线最早登录设备，并为已有设备账户的新设备登录接入服务器账号验证与已登录设备确认                                                                                                                                                                                                                            |
| 管理员后台首页  | 已提供 `/admin` 概览页，汇总待办和关键数字，帮助管理员优先处理审核、争议和告警；最近审计记录只在审计日志页查看                                                                                                                                                                                                                                                                       |
| 管理员用户审核  | 已提供用户目录、待审核列表、通过、拒绝、封禁、解封、删除、管理员介入重置用户密码、用户目录 CSV 导出的后台页面与 API                                                                                                                                                                                                                                                                  |
| 客户端应用      | 已提供管理员登记 `client_id`、回跳白名单、允许来源，登录页会校验回跳地址后返回外部项目，外部项目可按允许来源校验当前登录态                                                                                                                                                                                                                                                           |
| 添加卡券闭环    | 已提供管理员和发卡方生成领取码、添加链接、本地二维码、添加前预览、用户领取卡券、领取成功后直达卡券详情、未登录领取时引导登录后回到添加动作、首页读取真实钱包卡券，并在生成领取码时写入初始发放流水；`/add` 只接受领取码/添加链接，发卡方现场核销改为 `/provider/redemptions` 独立页面按已领取卡片卡号处理                                                                            |
...
| 发卡方发放      | 已提供发卡方基于已审核模板生成单个或批量领取码/添加链接、写入初始流水和审计事件的最小闭环；生成、批量复制、重发和领取码列表会展示对应卡号，支持实体卡制卡时建立领取码、添加链接与稳定卡号的对应关系；完整领取码统一在弹窗中一次性展示，关闭后列表只保留尾号                                                                                                                          |
| 开放提供方 API  | 已提供发卡方 API 密钥创建审批、审批通过后一次性查看明文密钥、密钥轮换/停用，以及 API 密钥发放领取码、查询/撤销/重发领取码、生成/查询/撤销操作链接、查询卡券、提交票券字段变更审核、调整权益、写接口签名、幂等响应复用和按密钥/scope 限流的最小闭环；发卡方后台列表已改为摘要 + 详情弹窗                                                                                              |
| 发卡方 Webhook  | 已提供发卡方提交回调端点申请、管理员审核通过后创建端点、发卡方一次性查看签名密钥、按事件选择投递、异步重试和配置审计事件的最小闭环；已支持领取码撤销/重发、操作链接创建/消费/过期/撤销、额度补充冲正、转赠状态变化、票券变更审核状态等事件订阅；端点详情和修改配置表单已改为弹窗                                                                                                     |
| 发卡方权益调整  | 已提供发卡方查看自己名下卡券、搜索卡券、调整金额/积分/次数、导出卡券与流水 CSV、写入流水和审计事件的最小闭环；卡券详情、调整余额、发起消耗、操作链接、状态管理、票券字段和票券变更记录已改为列表项操作按钮 + 弹窗承接                                                                                                                                                                |
| 操作链接管理    | 已提供发卡方生成确认使用/额度补充链接、按类型和状态筛选、查看当前或全部卡券链接、单条撤销和批量撤销 Active 链接，Next.js API application context 会定时标记过期链接，完整链接只在生成结果中展示一次                                                                                                                                                                                  |
...
- 管理员介入重置用户密码。已完成最小闭环：`GET /admin/users` 搜索用户目录，`POST /admin/users/:userId/password/reset` 写入新密码哈希并发布 `CredentialChanged` 审计事件；用户 PIN 保持用户自助设置和修改。
...
- 新设备登录验证。已完成服务器账号验证和已登录设备确认两个最小闭环：已有设备的账户从新设备登录时，后端可同时返回服务器聊天验证码和账户页确认请求，任一验证通过后才记录登录设备和创建会话；同系统超出 2 台活动设备时，最早登录的同系统设备会自动下线。
...
- `/provider/redemptions` 核销入口：发卡方专用登录态打开独立核销页，输入已领取卡片的完整卡号读取卡券并发起核销；领取码和添加链接只用于添加卡券，不再作为核销定位凭据。模板规则可配置允许核销方名单，默认只允许原发卡方自身核销。
...
- `/admin/users` 审核通过用户；在用户目录中搜索已激活用户，输入新密码、处置原因和管理员 PIN 后重置密码，`/admin/audit` 应出现 `CredentialChanged`。
...
- `/login` 使用审核通过的用户登录。
- 已有登录设备的服务器账号用户在新设备上登录时，应看到服务器聊天验证码和已登录设备确认提示；用对应服务器 ID 发送验证码并点击检查后才会创建会话，`/admin/audit` 应出现 `ServerVerificationCodeIssued`、`DeviceLoginVerified`、`LoginDeviceRecorded` 和 `UserLoggedIn`。
- 已有登录设备的用户在新设备上登录后，回到任一已登录设备打开 `/account`，应看到待确认的新设备登录请求；批准后回到新设备点击“已批准，检查”才会创建会话，`/admin/audit` 应出现 `DeviceLoginApprovalRequested`、`DeviceLoginApprovalApproved`、`LoginDeviceRecorded` 和 `UserLoggedIn`；拒绝后新设备继续检查应显示已被拒绝。
...
- 发卡方专用登录态打开 `/provider/redemptions?cardNumber=...` 或手动输入完整卡号，应读取已领取卡片并发起核销；当前发卡方是原发卡方或在模板允许核销名单中，且卡券已领取、未冻结/过期/归档时，才可发起待用户确认的核销请求。普通用户登录态打开 `/add` 时仍只能通过领取码/添加链接添加卡券，不能凭卡号领取。
...
本轮已先完成多项前后端闭环：管理员用户封禁/解封/匿名化删除、UTC+8 绝对时间主题计划、存储状态项目占用统计、核销场景服务器账号本次验证码确认、管理员后台统一导航壳、发卡方后台统一导航壳、提供方资料变更审批、Webhook 配置全生命周期审批。当前复核后确认仍需继续处理后台信息架构、统一身份模型、旧领取码兼容和部分视觉细节。

### 13.1 必须优先处理

1. 后台页面布局统一：管理员后台、发卡方后台和账户页已完成第一版“标题栏 + 主内容区”布局；账户页修改项已收敛为弹窗。管理员用户/卡券和发卡方工作台/发放/卡券/API/Webhook 已完成第一轮弹窗化，后续继续巡检其它长列表页面。
2. 后台导航统一：管理员后台和发卡方后台已完成上级首页/分类页导航；子页顶栏使用返回按钮 + 当前标题，移动端和平板端不再用横向 tab 承载主要分类。
...
8. 发卡方网页登录身份迁移：当前仍使用 `ProviderAccount`、`ProviderAuthSession` 和 `ldpass_provider_session`；目标是迁移为统一 `User + ProviderMembership`，普通会话承载用户、管理员和发卡方成员身份，`/account` 展示对应后台入口，发卡方 API 密钥继续作为机器身份保留。
9. 注销释放身份标识：新删除流程已匿名化释放用户名、邮箱和服务器 ID，并用稳定匿名代号保留审计可追溯性；后续需补旧数据检查，修复历史 `Deleted` 用户仍占用原邮箱或服务器 ID 的情况。
...
3. 完整领取码展示规则固定为生成时一次性展示，后续只展示尾号、状态和领取信息；如丢失则作废重发。新生成码固定为 `LD-` + 大写字母/数字，领取和预览接口要兼容测试期已经生成的旧码，不用新生成规则拒绝历史码。
