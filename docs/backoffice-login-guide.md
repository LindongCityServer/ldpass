...
管理员后台除 `/admin/login` 外，已统一使用“顶部标题栏 + 侧边导航 + 主内容区”的后台壳。后台子页面之间的跳转由统一导航负责，不再在页面标题或工具栏里重复放“用户审核 / 审计日志 / 工作台”这类跨页面链接；页面工具栏只保留当前页面命令，例如刷新、导出、新增、提交或检测。桌面端和平板端页面命令显示在顶部标题栏右侧，移动端显示为页面右下角浮动工具栏，按钮过多时自动换行。
...
发卡方后台除 `/provider/login` 和 `/provider/register` 外，已统一使用“顶部标题栏 + 侧边导航 + 主内容区”的后台壳。发卡方负责人登录后可以通过统一导航进入工作台、模板、发放、核销、卡券、争议、API 密钥和 Webhook 页面；页面工具栏只保留当前页面命令，例如新建模板、发放领取码、刷新、提交 API 密钥申请、提交 Webhook 端点或发起核销。桌面端和平板端页面命令显示在顶部标题栏右侧，移动端显示为页面右下角浮动工具栏。
