...
- `/add` 在普通用户登录态下只作为添加卡券入口，普通用户只能通过领取码或添加链接领取卡券，即使 URL 带有 `cardNumber` 也不启用核销模式；仅发卡方专用登录态或显式 `mode=redeem` 时才切换为核销模式，按传入或手输的卡号、领取码或添加链接定位可核销卡券并发起核销请求。如果同一浏览器同时存在普通用户 session 和发卡方 session，默认按普通用户添加流程处理，必须显式进入核销模式才能使用发卡方能力。发卡方核销模式下，领取码只作为定位 `passId` 的索引，即使领取码已经过期也可以定位对应卡片，但仍必须满足卡券已领取、未冻结、未过期、未归档以及当前发卡方在允许核销名单内等核销条件。
...
- 卡券模板可配置允许核销方名单；默认仅允许原发卡方核销。被授权的其他发卡方可在 `/add` 发卡方核销模式按 `publicNumber`、领取码或添加链接预览并发起核销，后端必须同时校验卡券状态、持卡用户、模板授权名单和幂等键。
...
| `UserPinResetByAdmin`                   | 管理员介入重置用户 PIN                 | 审计、账户安全               |
...
4. 管理员争议反转、管理员介入重置 PIN、异常账户处置的具体操作页面。
