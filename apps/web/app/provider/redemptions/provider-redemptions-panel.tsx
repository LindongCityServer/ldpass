...
export function ProviderRedemptionsPanel({ initialCardNumber }: ProviderRedemptionsPanelProps) {
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      const result = await getJson<ProviderRedemptionsResponse>('/api/provider/redemptions?take=10');
    ...
    <section className="admin-panel" aria-labelledby="provider-redemptions-title">
      <div className="admin-panel-heading">
        ...
        <div className="admin-list-actions">
          ...
          <button className="secondary-action" type="button" onClick={() => void loadHistory()}>
            刷新记录
          </button>
        </div>
      </div>
      ...
      {providerAccount ? (
        <>
          <form className="stacked-form" onSubmit={submitRedemption} noValidate>
            ...
            <div className="form-actions compact-actions">
              <button
                ...
                onClick={() => void loadPreview(cardNumberInput, { showReadyMessage: true })}
              >
                <span className="material-symbols-rounded" aria-hidden="true">
                  visibility
                </span>
            ...
            <label>
              ...
              <select
                ...
                onChange={(event) => setVerificationMethod(event.target.value as 'pin' | 'server_account')}
              >
          ...
          {!isHistoryLoading && redemptionRequests.length === 0 ? <p className="empty-note">暂无核销记录。</p> : null}
            ...
            {redemptionRequests.map((request) => (
              <article className="admin-list-item" key={request.id}>
                <div>
                  ...
                  <p>
                    卡号：{request.pass.maskedNumber ?? request.pass.publicNumber ?? request.pass.id} · 持卡用户：
                    {request.user ? `${request.user.username}（${request.user.email}）` : '未知'}
                  </p>
                  <p>
                    {formatRedemptionStatus(request.status)} · {formatVerificationMethod(request.verificationMethod)} ·{' '}
                    {formatBenefitValue(request.requestedValue, request.pass.benefitType)}
                  </p>
                  <p>
                    到期：{formatDate(request.expiresAt, '未知')} · 发起：{formatDate(request.createdAt, '未知')}
                  </p>
  ...
  return (
    <section className={`add-pass-preview add-pass-preview-${preview.pass.category}`} aria-label="待核销卡券信息">
      <div
        ...
        style={preview.pass.backgroundImageUrl ? { backgroundImage: `url("${preview.pass.backgroundImageUrl}")` } : undefined}
        aria-hidden="true"
      >
        <small>{formatPassTailNumber(preview.pass.maskedNumber ?? preview.pass.publicNumber) ?? '**** 5678'}</small>
      </div>
...
function normalizeCardNumberLookup(value: string): string {
  ...
  try {
    ...
    return (url.searchParams.get('cardNumber') ?? trimmedValue).replace(/\s+/g, '').trim().toUpperCase();
  } catch {
...
function formatBenefitValue(value: string, benefitType: ProviderRedemptionPass['benefitType']): string {
