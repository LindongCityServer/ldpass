...
export function AdminCardTemplateVariantsPanel() {
  ...
  const loadVariants = async () => {
    ...
    try {
      const result = await getJson<CardTemplateVariantsResponse>(
        '/api/admin/card-template-variants',
      );
  ...
  const createVariant = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await postJson<CardTemplateVariantResponse>(
        '/api/admin/card-template-variants',
        {
  ...
  const exportVariantsCsv = () => {
    ...
    downloadTextFile(
      'ldpass-admin-card-template-variants.csv',
      toCsv(rows),
  ...
  const updateVariant = async (event: FormEvent<HTMLFormElement>, variant: CardTemplateVariant) => {
    ...
    try {
      const result = await postJson<CardTemplateVariantResponse>(
        `/api/admin/card-template-variants/${variant.id}`,
        {
          name: String(form.get('name') ?? '').trim(),
          category: String(form.get('category') ?? ''),
          enabled: form.get('enabled') === 'on',
      ...
      setVariants((currentVariants) =>
        currentVariants.map((item) => (item.id === result.variant.id ? result.variant : item)),
      );
  ...
  const deleteVariant = async (variant: CardTemplateVariant) => {
    if (
      !window.confirm(
        `删除卡面模板变体「${variant.name}」吗？已经引用该 key 的历史模板不会被自动改写。`,
      )
    ) {
      return;
    }
    ...
    <>
      ...
      <section className="admin-panel" aria-labelledby="card-template-variants-title">
        ...
        {!isLoading && filteredVariants.length === 0 ? (
          <p className="empty-note">还没有匹配的卡面模板变体。</p>
        ) : null}
            ...
            <article className="admin-list-item admin-list-item-review" key={variant.id}>
              {editingVariantId === variant.id ? (
                <form
                  ...
                  onSubmit={(event) => void updateVariant(event, variant)}
                >
                  <div>
                    <h2>{variant.key}</h2>
