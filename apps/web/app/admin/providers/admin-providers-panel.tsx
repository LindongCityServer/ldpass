...
export function AdminProvidersPanel() {
  ...
  >([]);
  const [webhookChangeRequests, setWebhookChangeRequests] = useState<
    ProviderWebhookChangeRequest[]
  >([]);
  ...
  const loadProviders = async (nextKeyword = keyword) => {
    ...
    try {
      const result = await getJson<AdminProvidersResponse>(
        `/api/admin/providers?${search.toString()}`,
      );
  ...
  const loadProfileChangeRequests = async () => {
    ...
    try {
      const result = await getJson<ProviderProfileChangeRequestsResponse>(
        '/api/admin/providers/profile-change-requests',
      );
  ...
  const loadWebhookChangeRequests = async () => {
    ...
    try {
      const result = await getJson<ProviderWebhookChangeRequestsResponse>(
        '/api/admin/providers/webhook-change-requests',
      );
  ...
  const loadApiKeyChangeRequests = async () => {
    ...
    try {
      const result = await getJson<ProviderApiKeyChangeRequestsResponse>(
        '/api/admin/providers/api-key-change-requests',
      );
  ...
  const createProvider = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage(
        `已创建提供方 ${result.provider.name}，负责人账号 ${result.account.email} 可直接登录发卡方后台。`,
      );
      await loadProviders();
    } catch (error) {
  ...
  const rejectProfileChangeRequest = async (request: ProviderProfileChangeRequest) => {
    ...
    try {
      await postJson(`/api/admin/providers/profile-change-requests/${request.id}/reject`, {
        reason,
      });
  ...
  const approveWebhookChangeRequest = async (request: ProviderWebhookChangeRequest) => {
    if (
      !window.confirm(
        `确定通过 ${request.provider?.name ?? request.providerId} 的 Webhook ${formatWebhookChangeKind(request.kind)}申请吗？`,
      )
    ) {
      return;
    }
  ...
  const rejectWebhookChangeRequest = async (request: ProviderWebhookChangeRequest) => {
    ...
    try {
      await postJson(`/api/admin/providers/webhook-change-requests/${request.id}/reject`, {
        reason,
      });
  ...
  const approveApiKeyChangeRequest = async (request: ProviderApiKeyChangeRequest) => {
    if (
      !window.confirm(
        `确定通过 ${request.provider?.name ?? request.providerId} 的 API 密钥申请吗？`,
      )
    ) {
      return;
    }
  ...
  const rejectApiKeyChangeRequest = async (request: ProviderApiKeyChangeRequest) => {
    ...
    try {
      await postJson(`/api/admin/providers/api-key-change-requests/${request.id}/reject`, {
        reason,
      });
  ...
  const changeProviderGovernance = async (
    provider: AdminProvider,
    action: ProviderGovernanceAction,
  ) => {
    ...
    <>
      <BackofficeTopbarPageActions>
        <div className="admin-list-actions">
          ...
          <button
            ...
            type="button"
            onClick={() => void loadProviders(keyword)}
            title="刷新"
          >
            <span className="material-symbols-rounded" aria-hidden="true">
              refresh
      ...
      <section className="admin-panel" aria-labelledby="admin-providers-title">
        ...
        <div className="segmented-control" role="tablist" aria-label="发卡方管理视图">
          <button
            className={activeView === 'providers' ? 'is-selected' : undefined}
            type="button"
          ...
          <button
            className={activeView === 'profile' ? 'is-selected' : undefined}
            type="button"
          ...
          <button
            className={activeView === 'api' ? 'is-selected' : undefined}
            type="button"
          ...
          <button
            className={activeView === 'webhook' ? 'is-selected' : undefined}
            type="button"
        ...
        {activeView === 'profile' ? (
          <section
            ...
            aria-labelledby="provider-profile-change-review-title"
          >
            <div className="detail-section-heading">
              ...
              <button
                className="secondary-action"
                type="button"
                onClick={() => void loadProfileChangeRequests()}
              >
                刷新待审
              </button>
            </div>
            {isLoadingProfileChanges ? <p className="empty-note">正在读取资料变更申请。</p> : null}
            {!isLoadingProfileChanges && profileChangeRequests.length === 0 ? (
              <p className="empty-note">暂无待审核资料变更。</p>
            ) : null}
                ...
                <article className="admin-list-item" key={request.id}>
                  ...
                  <div className="admin-list-actions">
                    ...
                    >
                      ...
                      <span>
                        {profileChangeActionKey === `${request.id}:approve`
                          ? '通过中'
                          : '通过并应用'}
                      </span>
                    </button>
        ...
        {activeView === 'api' ? (
          <section
            className="admin-list-section"
            aria-labelledby="provider-api-key-change-review-title"
          >
            <div className="detail-section-heading">
              ...
              <button
                className="secondary-action"
                type="button"
                onClick={() => void loadApiKeyChangeRequests()}
              >
                刷新待审
              </button>
            </div>
            {isLoadingApiKeyChanges ? <p className="empty-note">正在读取 API 密钥申请。</p> : null}
            {!isLoadingApiKeyChanges && apiKeyChangeRequests.length === 0 ? (
              <p className="empty-note">暂无待审核 API 密钥申请。</p>
            ) : null}
            <div className="admin-list">
                ...
                <article className="admin-list-item" key={request.id}>
                  <div>
                    ...
                    <p>
                      提供方：
                      {request.provider
                        ? `${request.provider.name} (${request.provider.slug})`
                        : request.providerId}{' '}
                      · 状态：
                    ...
                    {request.kind === 'RevokeApiKey' ? null : (
                      <p>
                        权限：
                        {request.proposed.scopes
                          .map((scope) => formatApiKeyScope(scope))
                          .join('、')}
                      </p>
                    )}
                    {request.kind === 'RevokeApiKey' ? null : (
                      <p>
                        有效期：
                        {request.proposed.expiresAt
                          ? formatDateTime(request.proposed.expiresAt)
                          : '长期有效'}
                      </p>
                    )}
                    {request.reason ? <p>申请说明：{request.reason}</p> : null}
                  ...
                  <div className="admin-list-actions">
                    ...
                    >
                      ...
                      <span>
                        {apiKeyChangeActionKey === `${request.id}:approve`
                          ? '通过中'
                          : formatApiKeyApproveAction(request.kind)}
                      </span>
                    </button>
                  </div>
                </article>
        ...
        {activeView === 'webhook' ? (
          <section
            className="admin-list-section"
            aria-labelledby="provider-webhook-change-review-title"
          >
            <div className="detail-section-heading">
              ...
              <button
                className="secondary-action"
                type="button"
                onClick={() => void loadWebhookChangeRequests()}
              >
                刷新待审
              </button>
            </div>
            {isLoadingWebhookChanges ? (
              <p className="empty-note">正在读取 Webhook 配置申请。</p>
            ) : null}
            {!isLoadingWebhookChanges && webhookChangeRequests.length === 0 ? (
              <p className="empty-note">暂无待审核 Webhook 配置。</p>
            ) : null}
            <div className="admin-list">
                ...
                <article className="admin-list-item" key={request.id}>
                  ...
                  <div className="admin-list-actions">
                    ...
                    >
                      ...
                      <span>
                        {webhookChangeActionKey === `${request.id}:approve`
                          ? '通过中'
                          : formatWebhookApproveAction(request.kind)}
                      </span>
                    </button>
                  </div>
                </article>
        ...
        {activeView === 'providers' ? (
          <>
            <form className="audit-filter-grid" onSubmit={submitSearch}>
              ...
              <div className="audit-filter-actions">
                ...
                <button
                  ...
                  onClick={() => void exportProvidersCsv()}
                >
            ...
            {!isLoading && providers.length === 0 ? (
              <p className="empty-note">暂无提供方。</p>
            ) : null}
              ...
              {providers.map((provider) => (
                <article className="admin-list-item" key={provider.id}>
                  <div>
                    <h2>{provider.name}</h2>
                    <p>
                      标识：{provider.slug} · 状态：{formatProviderStatus(provider.status)} · 来源：
                      {formatProviderSource(provider.source)}
                    </p>
                    <p>
                      联系人：{provider.contactName ?? '未填写'} · 邮箱：
                      {provider.contactEmail ?? '未填写'}
                    </p>
                  </div>
                  <div className="admin-list-actions">
                    <button
                      className="secondary-action"
                      type="button"
                      onClick={() => setActiveDialog({ kind: 'detail', provider })}
                    >
                      详情
                    </button>
                    {isReviewableProvider(provider.status) ? (
                      <>
                        <button
                          className="secondary-action"
                          type="button"
                          onClick={() => void rejectProvider(provider.id)}
                        >
                          拒绝
                        </button>
                        <button
                          className="primary-action"
                          type="button"
                          onClick={() => void approveProvider(provider.id)}
                        >
                          <span className="material-symbols-rounded" aria-hidden="true">
                            check
                          </span>
            ...
            {activeDialog ? (
              <ProviderDialogPanel
                dialog={activeDialog}
                governanceInput={
                  governanceInputs[activeDialog.provider.id] ?? { reason: '', secondFactor: '' }
                }
                governanceActionKey={governanceActionKey}
                onClose={() => setActiveDialog(null)}
                onGovernanceInputChange={(patch) =>
                  updateGovernanceInput(activeDialog.provider.id, patch)
                }
                onGovernanceSubmit={() => {
...
function ProviderDialogPanel({
  ...
  const title =
    dialog.kind === 'detail' ? '发卡方详情' : formatProviderGovernanceAction(dialog.action);
        ...
        {dialog.kind === 'detail' ? (
          ...
          >
            ...
            <div className="admin-dialog-actions">
              ...
              >
                {governanceActionKey === `${dialog.provider.id}:${dialog.action}`
                  ? '处理中'
                  : formatProviderGovernanceAction(dialog.action)}
              </button>
            </div>
          </form>
        )}
